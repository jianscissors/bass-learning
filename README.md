...
| 🎵 曲目练习 | [songs.html](https://jianscissors.github.io/bass-learning/songs.html) | 几首爵士标准曲（Fly Me to the Moon / All the Things You Are / Blue Bossa / The Girl from Ipanema），按"找根音 → 跟根音 → 标级数和弦 → 加 chord tone"四阶段推进，练指板熟悉度 |

每个练习工具都有独立的"学习记录"面板（练习时长、正确率、连对等），存在浏览器本地，换设备不会同步。

## 技术

- 纯 HTML/CSS/JS，无构建步骤，`theme.css` 是七个工具共用的设计系统
